...
import { fetchGoogleSheetsData as fetchData } from './api'; // Using MySQL API now
...
export function AppProvider({ children }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (sheetData) {
                console.log('Data loaded from MySQL API:', sheetData);
                setData(sheetData);
            }
            setLoading(false);
        };
    ...
    const login = async (email, password) => {
        try {
            ...
            setUser({
                email: user.email,
                role: user.role,
                specialistId: user.specialistId || user.ID_ESPECIALISTA_LINK
            });
    ...
    const logout = () => {
        // Llamar logout de API para limpiar token
    ...
    const syncWithAPI = async (action, apiFunction, payload) => {
        try {
